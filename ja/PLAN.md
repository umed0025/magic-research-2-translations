...
翻訳の現状・導入方法など概要は [ja/README.md](README.md) を参照。このファイルは既知の落とし穴・検証手順・作業ログなど、作業を再開する人向けの詳細情報を扱う。

## 前提・経緯

- リポジトリ本来の公式窓口は [Localizor](https://www.localizor.com/magic-research-2/) に移行済み、とREADME.mdに記載されているが、**Localizor自体が閉鎖済みで現在アクセス不能**（2026-08-23確認）。`localizor.com`への現在のアクセスは301リダイレクトで `hovgaard.com` へ転送される（第三者への乗っ取りではなく、Localizorの開発・運営元であるゲームスタジオ Hovgaard Games（デンマーク、創業者Jonas Hovgaard）自身の公式サイトへの統合）。Hovgaard Gamesは2023年11月に自社ゲーム（Big Ambitions、Startup Company等）のパブリッシング事業へ軸足を移す方針転換を発表しており（[Games Press記事](https://www.gamespress.com/Hovgaard-Games-Goes-Into-Publishing)）、この流れの中でLocalizorという独立サービスの提供が終了したとみられる。Wayback Machineの記録では2026-03-10時点が最後の生存スナップショット（status 200）で、それ以降（4月・6月・8月の各時点で確認）新しいスナップショットが取れておらず、公式な「サービス終了しました」という一次アナウンスは確認できなかったが2026年3月頃に独立サイトとしての運用を終了したとみられる。README.mdの「Localizorへ移行済み」表記は現状と合わなくなっているため、**このリポジトリでのレガシー方式による翻訳が実質唯一の手段**という状況。この点を承知の上でユーザーがこのリポジトリでの翻訳続行を選択している。
...
**進捗: 4446 / 4646 件（翻訳完了。実質100%）**

残り200件は、バッチ2・3で意図的に据え置いたプレースホルダー・アイコン記法・固有名詞のみのキー（原文保持が正しいもの）。2026-08-23、200件全件を機械的に精査（`{{param}}`・`:token:`等のトークンを除去した残りに英単語が3文字以上残るか判定）し、該当した8件（キーボードショートカット表記、`Magic Research`/`Magic Research 2`、`MPL`表記、単位記号等）もすべて翻訳不要が正しいことを個別確認済み。翻訳漏れは無い。
...
## 用語集の確定訳と実際の訳文の突合チェック（2026-08-22実施）

用語集（[ja/glossary.md](glossary.md)）に記載の各英語用語について、base-translations.jsonでその用語を含むキーを抽出し、対応するja/translations.jsonの値が確定訳語を含んでいるかを機械チェックした。Home以外に以下3件の表記ブレを発見・修正済み。

- **Hypervault**: ストーリー内の命名シーン（「それを*ハイパーヴォールト*と呼ぶことにする」）では音訳が使われている一方、Storylineタイトル・実績・ボタン名の4箇所で意訳「大金庫」が使われていた。用語集の確定訳「ハイパーヴォールト」に統一。
- **Mad Alchemist**: 唯一翻訳済みだったキー「狂気の錬金術師」が用語集の確定訳「狂った錬金術師」とズレていた。統一。
- **Tracking Stone**: 翻訳済み2箇所が「追跡石」、用語集は「追跡の石」。他アイテム名の「〜の〜」パターン（Deflective Shield→偏向の盾等）に合わせ「追跡の石」に統一。

**チェック手法**: 用語集の英語見出し語ごとに「期待される日本語訳の候補」リストを作り、base-translations.jsonでその語を含むキーに対応するja値が候補のいずれも含まない場合を抽出。Challenge（"Final Challenge"→「最後の試練」）等の文脈的な意訳は誤検出として除外が必要。再実行する場合は同様のスクリプトを組んで用語集の項目を総当たりすること。
