...
| 4 | 長文ストーリー・ダイアログ（100文字超）。ネタバレを含む。真エンディング3種（贖罪/恐ろしい過ち/命を救うために）を含め完了 | 1071 / 1071 | 完了 |

**進捗: 4446 / 4646 件（約95.7%）**

残り200件は、バッチ2・3で意図的に据え置いたプレースホルダー・アイコン記法・固有名詞のみのキー（原文保持が正しいもの）。実質的な翻訳作業は完了。
...
3. **長文ストーリー・ダイアログ**（100文字超、約1071件）: ネタバレを含む。キャラの口調・文体統一が必要。これが最後の主要バッチ。**進行中（955/1071、残り約116件）**。チャンク単位（100〜120件ずつ、短い順）で処理を継続する。残り116件を処理すればバッチ4は完了見込み。
...
## 重大な落とし穴: UI操作名と説明文で訳語がブレる（2026-08-22発見）

"Home"のように、確定訳（例:「ホーム」＝Expand Homeアクション名）と、一般名詞としての訳（「自宅」「家」）の両方が成立する単語は、原文の同じ英単語（"your home"等）が文脈によって全然違う日本語になりうる。バッチ2〜4を通じて訳した際、UIのボタン名「ホームを拡張」に直接対応する強化・実績の説明文7件が「自宅の拡張」と訳されてしまっていた（ユーザー指摘で発覚）。プレイヤーはパネルの「ホームを拡張」ボタンと、説明文の「自宅の拡張」が同じ機能を指すとすぐには気づけない。

**教訓・判断基準**: その文がゲーム内の具体的な操作・ボタン・実績条件を説明しているか（→確定訳の「ホーム」を使う）、それとも物語上の情景を描写しているだけか（→「自宅」「家」で自然に訳してよい）で判断する。詳細な使い分けルールは[ja/glossary.md](glossary.md)の「Home の訳し分けルール」に記載。**同種の単語**（画面名・システム名としても一般名詞としても使われる語。例えば他に見つかっていないが、今後同様の単語が出た場合は同じ観点でチェックすること）が出た場合は、都度この基準で確認する。
