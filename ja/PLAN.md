...
1. **短いUIラベル・ボタン文言**（20文字以下、全体で約1659件）: 機械的に進めやすく、用語集の定着にも有効。
2. **Storyline名一覧**（`(Storyline) ...`で始まるキー、数百件規模）: 実績名。用語集と整合させながら進める。
3. **アイテム・呪文・モンスター名とその短い説明**: 個別の固有名詞が多く、都度用語集に追記が発生しやすい。
4. **長文ストーリー・ダイアログ**（100文字超、約1103件）: ネタバレを含む。キャラの口調・文体統一が必要なため最後に回すのが無難。
