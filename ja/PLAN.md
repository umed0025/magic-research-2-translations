...
| 4 | 長文ストーリー・ダイアログ（100文字超）。ネタバレを含む。チャンク単位（100〜120件ずつ）で継続中 | 840 / 約1071 | 進行中 |

**進捗: 4272 / 4646 件（約92.0%）**
...
3. **長文ストーリー・ダイアログ**（100文字超、約1071件）: ネタバレを含む。キャラの口調・文体統一が必要。これが最後の主要バッチ。**進行中（840/1071、残り約231件）**。チャンク単位（100〜120件ずつ、短い順）で処理を継続する。
